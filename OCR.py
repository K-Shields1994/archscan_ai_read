...
def start_gui(handle_folder_upload):
    ...
    title_label = tk.Label(header_frame, text="OCR File Processor", font=("Helvetica", 24, "bold"), fg='white', bg='#4a90e2')
    ...
    # Input Folder Button and Label
    input_frame = tk.Frame(button_frame, bg='#f0f0f0')
    input_frame.pack(side='left', padx=10)
    upload_button = tk.Button(input_frame, text="Select Input Folder", command=upload_folder, font=("Helvetica", 12), bg="#4a90e2", fg="white", padx=10, pady=5)
    upload_button.pack(side='top', padx=10)
    input_folder_label = tk.Label(input_frame, text="No input folder selected", font=("Helvetica", 12), bg="#f0f0f0")
    input_folder_label.pack(side='top', padx=10)

    # Output Folder Button and Label
    ...
    output_button.pack(side='top', padx=10)
    output_folder_label = tk.Label(output_frame, text="No output folder selected", font=("Helvetica", 12), bg="#f0f0f0")
    output_folder_label.pack(side='top', padx=10)

    # Run Button
    run_button_frame = tk.Frame(button_frame, bg='#f0f0f0')
    run_button_frame.pack(side='left', padx=10)
    run_button = tk.Button(run_button_frame, text="Run", command=run_process, font=("Helvetica", 12), bg="#4a90e2", fg="white", padx=20, pady=10)
    run_button.pack(side='top', padx=10)

    # Output Text Area
    output_text = scrolledtext.ScrolledText(main_frame, wrap=tk.WORD, width=75, height=25, font=("Courier", 10))
